function status = run_leg_worker(repo, tag, dose, mult, wallBudget)
...
try
    ...
    logdir = fullfile(repo,'calibration','logs');
    ...
    if leg_is_complete(repo, tag, dose)
        status = 'complete';
    else
    ...
    try
        f = fullfile(repo,'calibration','logs', sprintf('%s_dose%g_error.log', tag, dose));
        fid = fopen(f,'a');
